...
| ID | 事项 | 阻塞 | 我的默认 | 需要你做的 |
|---|---|---|---|---|
| O-01 | **你的资源**：是否一个人做？iOS/后端经验？每周可投入小时数？希望多久给同学试用？ | 时间线（§16） | 一人 + coding agent，会 Swift 基础，每周约 12 小时 | ✅ 已回答（2026-09-19）：T0.1 之后交由其他协作者接手后续实现，具体人力/进度以协作者评估为准；agent 之后只维护 `TODO.md`/本文档。**2026-09-19 更新**：改为由 Claude 继续实现 P0 阶段（T0.2 起），取代上面这条"交给其他协作者"的决定；仍按 `AGENTS.md` §0 流程逐 ticket 出计划、你确认后再写代码 |
...
| O-10 | 项目名称、Bundle ID、Apple Developer 团队 | T0.1 | 占位 `<App>` | 🟡 部分回答（2026-09-19）：项目名 **FridgeChef**；Bundle ID 暂维持占位 `com.fridgechef.app`（未定正式值）；Apple Developer Program 暂不注册付费账号，先用免费 Personal Team 真机测试——注意 TestFlight 分发必须有付费账号，Sign in with Apple（Spike S3）能否用免费账号需到时验证 |
